"""Snowflake profiler — query service and BaseProfiler implementation."""
...
# Snowflake system schemas that are always excluded regardless of config.
_SNOWFLAKE_SYSTEM_SCHEMAS: frozenset[str] = frozenset({"INFORMATION_SCHEMA"})
...
def _make_json_safe(value: Any) -> Any:
    ...
    snowflake-connector-python returns typed Python objects for many
    Snowflake types.  This converts them to primitives that Pydantic and
    ``json.dumps`` can handle without custom encoders.
    """
    if value is None:
        return None
...
class SnowflakeProfiler:
    """Orchestrates metadata extraction from a connected Snowflake database.
    ...
    Usage::
        ...
        async with create_connector(sf_config) as db:
            result = await SnowflakeProfiler(db, profiling_config).profile()
    """
    ...
    async def _fetch_schema_names(self) -> list[str]:
        ...
        By default, all non-system schemas in the database are profiled.
        Pass ``include_schemas`` in :class:`ProfilingConfig` to restrict
        to a specific set.
        """
        with tracer.start_as_current_span("profiler.fetch_schema_names"):
            exclude = list(_SNOWFLAKE_SYSTEM_SCHEMAS | set(self._config.exclude_schemas))
    ...
    async def _profile_table(self, table_row: dict[str, Any], schema_name: str) -> dict[str, Any]:
        ...
        with tracer.start_as_current_span("profiler.profile_table") as span:
            ...
            if self._config.include_indexes:
                ...
                # return an empty list to satisfy the schema contract.
    ...
    async def _fetch_columns(self, schema: str, table: str) -> list[dict[str, Any]]:
        ...
        Snowflake does not have native enum types; ``enum_values`` is
        always ``None``.
        """
    ...
    async def _fetch_row_count(self, schema: str, table: str) -> int | None:
        """Return row count from INFORMATION_SCHEMA, falling back to exact COUNT(*).

        ``ROW_COUNT`` may be ``NULL`` for freshly created or external tables.
        """
    ...
    async def _fetch_indexes(self) -> list[dict[str, Any]]:
        """Snowflake uses automatic micro-partitioning instead of indexes.
    ...
    async def _fetch_data_freshness(self, schema: str, table: str) -> dict[str, Any] | None:
        ...
        Snowflake does not have PostgreSQL's analyze/vacuum system.  The
        available timestamps are mapped to the shared ``DataFreshnessInfo``
        fields as the closest semantic equivalents:

        * ``last_analyze``     ← ``last_altered``  (last DDL/DML change)
        * ``last_vacuum``      ← ``last_ddl``      (last DDL operation)
    ...
    async def _fetch_sample_data(self, schema: str, table: str) -> list[dict[str, Any]]:
        ...
        Schema and table identifiers are double-quoted to prevent SQL
        injection.  They are **not** passed as parameters because Snowflake
        does not allow identifiers to be parameterised.
        """
        safe_schema = schema.replace('"', '""')
        safe_table = table.replace('"', '""')
        query = f'SELECT * FROM "{safe_schema}"."{safe_table}" ORDER BY RANDOM() LIMIT %s'
    ...
    def _esc(name: str) -> str:
        """Escape a Snowflake identifier (double-quote safe)."""
        return name.replace('"', '""')
    ...
    async def _fetch_numeric_stats(self, fqt: str, columns: list[dict[str, Any]]) -> dict[str, NumericColumnStats]:
        ...
        for col in columns:
            ...
            parts.extend(
                [
                    f'MIN("{c}")::FLOAT AS "{c}__min"',
                    f'MAX("{c}")::FLOAT AS "{c}__max"',
                    f'AVG("{c}")::FLOAT AS "{c}__mean"',
                    f'STDDEV("{c}")::FLOAT AS "{c}__stddev"',
                    f'VARIANCE("{c}")::FLOAT AS "{c}__variance"',
                    f'SUM("{c}")::FLOAT AS "{c}__sum"',
        ...
        for col in columns:
            ...
            parts.extend(
                [
                    ...
                    f'AVG(LENGTH("{c}"))::FLOAT AS "{c}__avg_length"',
    ...
    async def _fetch_temporal_stats(self, fqt: str, columns: list[dict[str, Any]]) -> dict[str, TemporalColumnStats]:
        ...
        for col in columns:
            ...
            parts.extend(
                [
                    f'MIN("{c}")::VARCHAR AS "{c}__min"',
                    f'MAX("{c}")::VARCHAR AS "{c}__max"',
                ]
            )
        query = f"SELECT {', '.join(parts)} FROM {fqt}"
        row = await self._db.fetch_one(query)
    ...
    async def _fetch_top_values(self, fqt: str, col_name: str, total_count: int) -> list[TopValueEntry]:
        ...
        query = (
            f'SELECT "{c}"::VARCHAR AS "value", COUNT(*) AS "count" '
            f"FROM {fqt} "
...
class SnowflakeProfilerService(BaseProfilerService):
    """Profiler for Snowflake databases.

    Delegates query execution to :class:`SnowflakeProfiler` and
    assembles the typed response from its raw dict output.
    """

    service_name = "Snowflake"
    span_name = "profiler.snowflake"
    _datasource_type = "snowflake"
    ...
    async def _run(
        ...
        return await SnowflakeProfiler(connector, config).profile()
