"""Redshift profiler — query service and BaseProfiler implementation."""
...
def _make_json_safe(value: Any) -> Any:
    ...
    redshift_connector returns typed Python objects for many Redshift types.
...
class RedshiftProfiler:
    """Orchestrates metadata extraction from a connected Redshift database.
    ...
    Usage::
        ...
        async with create_connector(redshift_config) as db:
            service = RedshiftProfiler(db, profiling_config)
    ...
    async def _fetch_row_count(self, schema: str, table: str) -> int | None:
        ...
        """
        row = await self._db.fetch_one(q.ROW_COUNT_FAST, (table, schema))
        if row is None:
            return None
        estimated = int(row["row_count"])
        if estimated >= 0:
            return estimated
        # reltuples = -1 means no statistics yet — fall back to exact count.
    ...
    async def _fetch_indexes(self) -> list[dict[str, Any]]:
        """Redshift uses sort/distribution keys instead of traditional indexes."""
        return []
    ...
    async def _fetch_foreign_keys(self, schema: str, table: str) -> list[dict[str, Any]]:
        ...
        Note: Redshift foreign keys are informational only and not enforced.
        """
    ...
    async def _fetch_data_freshness(self, schema: str, table: str) -> dict[str, Any] | None:
        """Fetch last analyze timestamp from stl_analyze."""
    ...
    async def _fetch_sample_data(self, schema: str, table: str) -> list[dict[str, Any]]:
        ...
        They are **not** passed as parameters because Redshift does not allow
    ...
    def _esc(name: str) -> str:
        """Escape a Redshift identifier (double-quote safe)."""
        return name.replace('"', '""')
    ...
    async def _fetch_string_stats(self, fqt: str, columns: list[dict[str, Any]]) -> dict[str, StringColumnStats]:
        ...
        for col in columns:
            ...
            parts.extend(
                [
                    f'MIN(LEN("{c}")) AS "{c}__min_length"',
                    f'MAX(LEN("{c}")) AS "{c}__max_length"',
                    f'AVG(LEN("{c}"))::float8 AS "{c}__avg_length"',
    ...
    async def _fetch_boolean_stats(self, fqt: str, columns: list[dict[str, Any]]) -> dict[str, BooleanColumnStats]:
        ...
        for col in columns:
            ...
            parts.extend(
                [
                    f'SUM(CASE WHEN "{c}" = TRUE THEN 1 ELSE 0 END) AS "{c}__true_count"',
                    f'SUM(CASE WHEN "{c}" = FALSE THEN 1 ELSE 0 END) AS "{c}__false_count"',
                ]
            )
        query = f"SELECT {', '.join(parts)} FROM {fqt}"
        row = await self._db.fetch_one(query)
...
class RedshiftProfilerService(BaseProfilerService):
    """Profiler for Amazon Redshift databases.

    Delegates query execution to :class:`RedshiftProfiler` and
    assembles the typed response from its raw dict output.
    """

    service_name = "Redshift"
    span_name = "profiler.redshift"
    ...
    async def _run(
        ...
        return await RedshiftProfiler(connector, config).profile()
