"""Unit tests for Redshift column statistics."""
...
from core.services.redshift.profiler import RedshiftProfiler
...
def _make_profiler(
    *,
...
) -> tuple[RedshiftProfiler, AsyncMock]:
    """Create a RedshiftProfiler with a mocked connector."""
    connector = AsyncMock()
    ...
    return RedshiftProfiler(connector, config), connector
...
class TestFetchColumnStatsCommon:
    ...
    async def test_returns_common_stats_for_all_columns(self):
        ...
        columns = [
            ...
            {"name": "name", "data_type": "character varying", "enum_values": None},
        ]
...
class TestFetchNumericStats:
    ...
    @pytest.mark.asyncio
    async def test_uses_case_when_not_filter(self):
        """Redshift uses SUM(CASE WHEN ...) instead of COUNT(*) FILTER (WHERE ...)."""
        profiler, connector = _make_profiler()
        connector.fetch_one = AsyncMock(
            return_value=_mock_row(
                {
        ...
        await profiler._fetch_numeric_stats('"s"."t"', [{"name": "val", "data_type": "integer"}])

        query: str = connector.fetch_one.call_args[0][0]
        assert "SUM(CASE WHEN" in query
...
class TestFetchStringStats:
    ...
    async def test_returns_string_length_stats(self):
        ...
        result = await profiler._fetch_string_stats('"s"."t"', [{"name": "email", "data_type": "character varying"}])
    ...
    @pytest.mark.asyncio
    async def test_uses_len_function(self):
        """Redshift uses LEN() instead of LENGTH()."""
        profiler, connector = _make_profiler()
        connector.fetch_one = AsyncMock(
            return_value=_mock_row(
                {
        ...
        await profiler._fetch_string_stats('"s"."t"', [{"name": "col", "data_type": "varchar"}])

        query: str = connector.fetch_one.call_args[0][0]
        assert "LEN(" in query

    @pytest.mark.asyncio
    async def test_uses_case_when_for_empty_count(self):
        """Redshift uses SUM(CASE WHEN ...) instead of COUNT(*) FILTER (WHERE ...)."""
        profiler, connector = _make_profiler()
        connector.fetch_one = AsyncMock(
            return_value=_mock_row(
...
class TestFetchBooleanStats:
    ...
    async def test_uses_case_when_not_filter(self):
        """Redshift uses SUM(CASE WHEN ...) instead of COUNT(*) FILTER (WHERE ...)."""
        profiler, connector = _make_profiler()
        connector.fetch_one = AsyncMock(return_value=_mock_row({"flag__true_count": 5, "flag__false_count": 5}))

        await profiler._fetch_boolean_stats('"s"."t"', [{"name": "flag", "data_type": "boolean"}])
...
class TestFetchTopValues:
    ...
    async def test_uses_percent_s_placeholder(self):
        """Redshift uses %s placeholders for LIMIT."""
        profiler, connector = _make_profiler(top_values_limit=5)
        connector.fetch_all = AsyncMock(return_value=[])

        await profiler._fetch_top_values('"s"."t"', "col", 100)

        query: str = connector.fetch_all.call_args[0][0]
...
class TestFetchOutlierCount:
    ...
    @pytest.mark.asyncio
    async def test_uses_percent_s_placeholder(self):
        """Redshift uses %s placeholders for outlier bounds."""
        profiler, connector = _make_profiler()
        connector.fetch_one = AsyncMock(return_value=_mock_row({"outlier_count": 0}))

        await profiler._fetch_outlier_count('"s"."t"', "val", 10.0, 90.0)
...
class TestEscapeIdentifier:
    def test_plain_name(self):
        assert RedshiftProfiler._esc("users") == "users"

    def test_double_quote_escaped(self):
        assert RedshiftProfiler._esc('my"table') == 'my""table'
